...
# This script creates a disk image with ptrace test suite for StarryOS.
...
IMAGE_SIZE_MB=1024
MOUNT_POINT="/mnt"
ROOTFS_DIR="alpine-rootfs-ptrace-tests"
DEST_DIR="$PROJECT_ROOT/arceos"
DEST_IMAGE_NAME="disk.img"
PTRACE_TESTS_BIN="$PROJECT_ROOT/userspace/ptrace-tests/target/aarch64-unknown-linux-musl/release/ptrace-tests"
ALPINE_TARBALL="$PROJECT_ROOT/alpine-minirootfs.tar.gz"
# --- End Configuration ---

# Build ptrace-tests as regular user before requiring sudo
if [ "$(id -u)" -eq 0 ]; then
...
echo "Compiling ptrace-tests as regular user..."
cd "$PROJECT_ROOT/userspace/ptrace-tests"
cargo build --release --target aarch64-unknown-linux-musl

echo "ptrace-tests compiled."

# Check if binary was built successfully
if [ ! -f "$PTRACE_TESTS_BIN" ]; then
  echo "Error: ptrace-tests binary not found at $PTRACE_TESTS_BIN" >&2
  exit 1
fi
...
echo "Copying ptrace-tests binary into rootfs directory..."
mkdir -p "${ROOTFS_DIR}/usr/bin"
cp "$PTRACE_TESTS_BIN" "${ROOTFS_DIR}/usr/bin/"
