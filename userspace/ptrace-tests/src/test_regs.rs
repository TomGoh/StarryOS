...
/// Test PTRACE_GETREGS - basic functionality
pub fn test_getregs_basic() -> TestResult {
    print_test_header("PTRACE_GETREGS - Basic");
...
/// Test PTRACE_GETREGS during syscall
pub fn test_getregs_during_syscall() -> TestResult {
    print_test_header("PTRACE_GETREGS - During Syscall");
    ...
    match process::fork() {
        ...
        Ok(child_pid) => {
            ...
            let regs = match ptrace::getregs(child_pid) {
                ...
                Err(e) => {
                    let _ = process::kill(child_pid, libc::SIGKILL);
                    return Err(format!("PTRACE_GETREGS at syscall entry failed: {}", e));
                }
            };
            ...
            // Expected syscall numbers for getpid on AArch64
            ...
            // Get registers at syscall exit
            let regs_exit = match ptrace::getregs(child_pid) {
                Ok(r) => r,
                Err(e) => {
                    let _ = process::kill(child_pid, libc::SIGKILL);
                    return Err(format!("PTRACE_GETREGS at syscall exit failed: {}", e));
                }
            };
            ...
            if retval <= 0 {
                let _ = process::kill(child_pid, libc::SIGKILL);
                return Err(format!("getpid should return positive PID, got: {}", retval));
            }
...
pub fn run_all_tests() -> (usize, usize) {
    let tests: Vec<(&str, fn() -> TestResult)> = vec![
        ("PTRACE_GETREGS - Basic", test_getregs_basic),
        ("PTRACE_GETREGS - During Syscall", test_getregs_during_syscall),
